...
def _verify_business_database(path: Path) -> None:
    ...
    if version != 14 or result != "ok" or requests is None or extraction_requests is None:
...
def verify_backup(backup: Path) -> dict[str, Any]:
    ...
    if not isinstance(record, dict) or record.get("schema") != 1 or record.get("business_schema") != 14:
