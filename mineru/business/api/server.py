...
from ..services import DocumentWorkflow, EvidenceReader
...
def build_app(config: ServerConfig) -> FastAPI:
    ...
    return create_app(workflow=workflow, store=store, evidence_reader=EvidenceReader(store=store, doclib=doclib))
