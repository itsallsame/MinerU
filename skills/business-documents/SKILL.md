...
Set `MINERU_BUSINESS_API_URL` to the approved business Web/API address, for example `http://127.0.0.1:8080` when running on the host. The script accepts only loopback/RFC1918 or IPv6 unique-local IP addresses, or an internal service name whose DNS answers all fall in those ranges; it connects to the checked address rather than re-resolving the name. It rejects other IP literals, non-internal hostnames, and HTTPS/credential URLs. A hostname suffix or this client check alone does not prove network isolation. The service is **open**: there is no login, user identity, role, token, or permission check. Network reachability is the deployment boundary. Confirm the configured address is the intended isolated service before uploading a document; do not invent an internet endpoint or silently switch servers.
...
python3 skills/business-documents/scripts/business_documents.py search-blocks REVISION_ID "年度通知"
...
`search` is business-scoped discovery: returned snippets are **current index previews, unconfirmed**, not frozen evidence or citation-ready text. Only hits mapped to a business document with a completed matching-tier revision are returned; `scan_complete` concerns the Doclib-reported result window, not guaranteed full-corpus recall. `search-pages` scans at most 25 pages of a **specified historical business revision** per call and returns page locators with `historical_parse_unconfirmed` snippets. `search-blocks` scans the same historical revision in bounded 25-page windows, returning top-level block locators and machine snippets; it fails rather than silently truncating too-dense pages. Continue either command from `next_page` with `--start-page`; one empty window is not a full-document negative. Page or block hits are **candidate references, not frozen evidence or confirmed facts**. The business Web can explicitly freeze a chosen block's historical text into an evidence snapshot. `read` takes a **business revision ID and its matching locator**; it reads that historical parse batch, returns `next_locator` for bounded continuation, and labels the text `historical_parse_unconfirmed`. Build a page locator from the revision's returned `short_id` and `tier` (or reuse a locator returned by search/outline/structure); never infer `short_id` from the file SHA-256 prefix. For a final citation, use a frozen evidence ID or confirmed field evidence, not a search preview.
...
For a long document, read progressively instead of dumping every page into context. Example: first run `revisions DOCUMENT_ID` and choose the returned historical `REVISION_ID`; run `outline REVISION_ID` for the first 25-page window and, only if its response has `next_page`, run `outline REVISION_ID --start-page NEXT_PAGE`. Search a specific question with `search-blocks REVISION_ID "risk term"`; continue using that response's `next_page` when necessary. Read a returned locator with `read REVISION_ID 'RETURNED_LOCATOR'`, then follow `next_locator` only when more of that passage is needed. Keep the same revision ID throughout, do not manufacture locators or interpret an unscanned window as no match. These are unconfirmed machine reads; for a final supported claim, ask the user to review/freeze the relevant passage in the Web, then use `evidence EVIDENCE_ID` or a reviewed `result RESULT_ID`.
...
Current boundary: the tree reflects nodes actually present in MinerU's model schema; do not invent table cells or exact child coordinates the model did not provide. Do not silently fall back to Doclib/official Skill; tell the user when requested evidence is unavailable.
