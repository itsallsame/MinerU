...
.venv/bin/python scripts/evaluate_business_samples.py \
  ...
  --output /独立受控报告目录/metrics.json
```

工具只发 GET 请求，不修改业务系统。先校验四类样本、源文件 SHA-256、业务文档/修订/提取运行身份与完成状态；任一不符则停止，不输出“部分通过”的总报告。它统计精确字段值召回、候选精确率、证据快照命中、可选标题召回以及未解决阻断问题，分别给总计、各类和场景标签分组。缺少分母的比率为 `null`，不填猜测值。证据命中同时要求候选值确实在冻结片段中；若人工标注给出原文片段/页码，还须匹配它们。报告不包含原文、字段值或冻结证据文本；样本 ID 必须自行选用无敏感信息的编码。
