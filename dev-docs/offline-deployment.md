...
- 运行数据：Doclib 的 `MINERU_HOME` 持久挂载；共享原文件目录 `/srv/mineru-inbox` 在 worker 中只读、未来在业务 API 中可写。业务数据目录另行持久挂载，三者都不进入代码镜像。
...
5. 设置 `MINERU_WORKER_IMAGE`、`MINERU_MODELS_HOST_DIR`、`MINERU_MODEL_MANIFEST_HOST_FILE`、`MINERU_DOCLIB_HOST_DIR`、`MINERU_SHARED_DOCUMENTS_HOST_DIR` 后运行 `docker compose -f docker/compose.business.yaml config` 检查配置。仅在目录和文件权限核对后启动。
