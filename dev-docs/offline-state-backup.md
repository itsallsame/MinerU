...
业务数据库目前只接受 schema 15；旧 schema 14 及更早版本不迁移。当前备份脚本也只验证 schema 15，且要求上传、手动提取和任务重试三个请求键表，不能用于把旧库转换成新库。QA-033 数据备份恢复和 QA-034 版本回退仍须在目标隔离麒麟机器上以实际制品、模型、GPU 和受控样本完成演练后才能标通过。
