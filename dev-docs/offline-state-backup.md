...
业务数据库目前只接受 schema 18；旧 schema 17 及更早版本不迁移。当前备份脚本也只验证 schema 18，且要求上传、手动提取、任务重试、任务取消、模板写入和人工复核写入六个请求键表，不能用于把旧库转换成新库。QA-033 数据备份恢复和 QA-034 版本回退仍须在目标隔离麒麟机器上以实际制品、模型、GPU 和受控样本完成演练后才能标通过。
